  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        console.log('paidFinesRes:', paidFinesRes);
    console.log('newFinesRes:', newFinesRes);
    console.log('pendingDisputesRes:', pendingDisputesRes);
    console.log('outstandingFinesRes:', outstandingFinesRes);
   // console.log('trendRes:', trendRes);

    if (paidFinesRes.error) throw new Error('Failed to fetch paid fines.');
    if (newFinesRes.error) throw new Error('Failed to fetch new fines.');
    if (pendingDisputesRes.error) throw new Error('Failed to fetch pending disputes.');
    if (outstandingFinesRes.error) throw new Error('Failed to fetch outstanding fines.');
   // if (trendRes.error) throw new Error('Failed to fetch trend data.');
